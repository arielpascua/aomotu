...
const PARTICLE_COLOR = "rgba(255, 200, 87, 0.75)";
const NODE_LINE_COLOR = "rgba(122, 60, 251, 0.18)";
const MOUSE_LINE_COLOR = "rgba(255, 200, 87, 0.4)";
...
class SparkParticle {
  constructor(x, y) {
    ...
    this.color = Math.random() > 0.4 ? "#ffc857" : "#7a3cfb";
  }
  ...
  for (let i = 0; i < canvasParticles.length; i++) {
    ...
    if (mouse.active) {
      ...
      if (dist < mouse.radius) {
        ...
        canvasContext.strokeStyle = `rgba(255, 200, 87, ${alpha})`;
    ...
    for (let j = i + 1; j < canvasParticles.length; j++) {
      ...
      if (dist < connectDist) {
        ...
        canvasContext.strokeStyle = `rgba(122, 60, 251, ${alpha})`;
